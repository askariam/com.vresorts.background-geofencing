...
public class Geotracker extends Service implements LocationListener {
    ...
    private static final Integer MAX_STATIONARY_ACQUISITION_ATTEMPTS = 1;
    ...
    private static final class RegisteredTripPlan extends TripPlan{
        ...
        public JSONObject toJSONObject(){
    		...
    		try {
    			object.put("uuid", this.getUuid());
    			object.put("trip_plan_name", this.getTripPlanName());
    			object.put("user_uuid", this.getUserUuid());
    			
    			JSONArray placeJSONArray = new JSONArray();
    			for(Place place : this.getRegisteredPlaces()){
        		...
        		if(command != null && command.equals(GEOTRACKER_COMMOND_CONFIGURE_TRIPPLAN)){
        			...
        			if(tripplanData != null){
        				...
        				if(tripPlanToConfigure != null){
        					...
        					if(isDebugging){
        						Log.v(TAG, tripplan.toJSONObject().toString());
        					}
        				}
        			}
    	...
    	for(RegisteredPlace place : placeList){
    		...
    		if(distance < place.getRadious() + location.getAccuracy()){
    			...
    			if(place.wasLocatedIn){
    				onExit(place);
    				if(isDebugging){
            			Log.v(TAG, "Exit place:"+place.getPlaceName());
            		}
    			}
